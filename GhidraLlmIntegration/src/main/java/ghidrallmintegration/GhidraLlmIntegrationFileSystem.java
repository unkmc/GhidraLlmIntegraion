...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = GhidraLlmIntegrationFileSystem.MyFileSystemFactory.class)
public class GhidraLlmIntegrationFileSystem implements GFileSystem {
	 ...
	 */
	public GhidraLlmIntegrationFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + GhidraLlmIntegrationFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<GhidraLlmIntegrationFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public GhidraLlmIntegrationFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			GhidraLlmIntegrationFileSystem fs = new GhidraLlmIntegrationFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
